...
class ContainerRedisAgentApplicationTest {

    @Test
    void configUsesRedisStateAndAgentScope2DockerSandbox() throws Exception {
        ...
        assertThat(properties.getProperty("spring.application.name"))
                .isEqualTo("liteflow-agent-web-container-redis");
        ...
        assertThat(properties.getProperty("liteflow.agent.session-store.redis.uri"))
                .isEqualTo("redis://localhost:6379/0");
        assertThat(properties.getProperty("liteflow.agent.session-store.redis.key-prefix"))
                .isEqualTo("liteflow-agent-web-container-redis:");
        assertThat(properties.getProperty("liteflow.agent.harness.docker.snapshot-root")).isNull();
    ...
    void applicationDoesNotPrintAgentStreamEventsDirectly() {
        boolean hasPrintEventMethod = Arrays.stream(ContainerRedisAgentApplication.class.getDeclaredMethods())
                .map(Method::getName)
