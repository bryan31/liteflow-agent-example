# liteflow-agent-web-local-mysql

Agent 在宿主机执行的 LiteFlow Agent Web 示例，端口为 **8903**。会话、历史消息、工作区文件、Harness 记忆与附件统一保存在 MySQL。执行目录位于配置的 `harness.local.workspace-root` 下。`local` 表示 Agent 使用本地执行后端，MySQL 默认通过 Docker Compose 启动。
...
支持流式聊天、历史会话、Skill 加载、工作区文件读写和 `deliver_artifact` 附件下载。本示例已启用 `execute`，可调用宿主机的 Shell、`python3` 和 `node`，子 Agent 仍默认禁用。Agent 执行无需沙箱镜像；启动本模块的 MySQL 容器需要 Docker。
...
Compose 使用 `mysql:8.4`，监听 `127.0.0.1:3306`，首次启动自动创建 `liteflow_agent_local` 数据库和 `liteflow` 用户，密码为 `liteflow_example`，与应用默认配置一致，无需手动执行 `init.sql`。应用会自动创建所需表。

MySQL 数据保存在本模块 Compose 项目的 `mysql-data` 卷中。`docker compose -f liteflow-agent-web-local-mysql/docker-compose.yml down` 保留数据卷，追加 `-v` 会删除数据。初始化环境变量只在空数据卷上生效。

如需连接已有的本机或远程 MySQL，可跳过 Compose。首次准备默认数据库时，由有权限的账户执行本模块的 `init.sql`（其中的账户适用于本机连接；远程连接需配置相应账户授权）：

```bash
mysql -u root -p < liteflow-agent-web-local-mysql/init.sql
```

以下环境变量可覆盖默认连接；使用 Compose 时，在启动数据库和应用前在同一终端设置：

```bash
export MYSQL_JDBC_URL='jdbc:mysql://localhost:3306/liteflow_agent_local'
export MYSQL_DATABASE='liteflow_agent_local'
export MYSQL_USER='liteflow'
export MYSQL_PASSWORD='liteflow_example'
```

`MYSQL_DATABASE` 应与 JDBC URL 中的数据库一致。默认库名与运行命名空间独立于 `web-container-mysql` 模块；连接已有数据库时，请使用对应的账号与密码。两个模块的 Compose 默认都占用宿主机 3306 端口，不能同时绑定；如需并行运行，可修改其中一个 Compose 的宿主机端口，并同步修改对应的 JDBC URL。
...
访问 [http://localhost:8903](http://localhost:8903)。可通过 `DEEPSEEK_BASE_URL` 和 `DEEPSEEK_MODEL` 调整模型地址和名称，默认值与 Docker 示例相同。也可以将 `web-container-mysql` 模块的 `src/main/resources/application-local.yml` 复制到本模块同名目录，复用模型凭据与配置；其中显式填写的值优先于主配置中的环境变量占位符。该本机配置文件已被 `.gitignore` 排除。
...
```bash
mvn -pl liteflow-agent-web-local-mysql -am clean package
node --test liteflow-agent-web-local-mysql/src/test/js/*.test.mjs
```

单元测试不调用真实模型，也不要求启动数据库。完整的本地执行 HTTP 验收使用本地模拟模型，实际执行 Shell、Python、Node，验证脚本生成文件、读取、附件下载和应用重启恢复：

```bash
python3 scripts/web_local_acceptance.py --backend mysql
```

验收脚本使用 `scripts/fixtures/skills/local-report` 测试资源，不会调用外部行情接口。

MySQL 验收需要设置 `MYSQL_JDBC_URL`、`MYSQL_DATABASE`、`MYSQL_USER` 和 `MYSQL_PASSWORD`，连接专用测试库；脚本使用随机命名空间。验收数据与日志保存在模块 `target/local-acceptance-*` 下。
