...
class LocalMysqlAgentApplicationTest {

    @Test
    void configUsesMysqlStateAndAgentScope2GuardedLocalFilesystem() throws Exception {
        ...
        assertThat(properties.getProperty("spring.application.name"))
                .isEqualTo("liteflow-agent-web-local-mysql");
        ...
        assertThat(properties.getProperty("liteflow.agent.session-store.mysql.jdbc-url"))
                .isEqualTo("${MYSQL_JDBC_URL:jdbc:mysql://localhost:3306/liteflow_agent_local}");
    ...
    void applicationDoesNotPrintAgentStreamEventsDirectly() {
        boolean hasPrintEventMethod = Arrays.stream(LocalMysqlAgentApplication.class.getDeclaredMethods())
                .map(Method::getName)
